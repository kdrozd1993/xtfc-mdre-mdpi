...
t00 = [0; 0.4]; % Initial time
T = [0.01; 0.001]; % IVP time horizon
tff = [0.4; 0.5]; % Final time
nT = round(sum((tff - t00)./T)); % Number of propagations
timeStore = [t00(1)];
for i = 1:length(T)
    tt = t00(i):T(i):tff(i);
    tt(1) = [];
    timeStore = [timeStore, tt];
end
...
c = (tau(end)-tau(1))/(T(end));
t = t00(end) + (1/c)*(tau - tau(1)); % Collocation points in the time domain
...
timeInterpStore(end) = timeStore(end);
...
timeInterp = unifrnd(t00(1), tff(end), 50, 1);
...
save('mdle_xtfc.mat', 'results', 'options');
